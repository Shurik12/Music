...
|---|---------|--------|----------------|----------------------|
| 1 | `list` | List playlists | `list_playlists` (src/cli.py:56) → `YTMusicClient.get_playlists/print_playlists` (src/ytmusic.py:108,125) | Prints `title: playlistId` (up to 100 playlists) |
| 2 | `artists` | Get playlist artists | src/cli.py:64 → `get_playlist/get_playlist_artists` (src/ytmusic.py:163,171) | **Hardcoded to `playlists[1]`** — effectively a debug command |
| 3 | `tracks` | Count liked tracks not in any playlist | src/cli.py:133 → `get_track_out_playlist` (src/ytmusic.py:270) | Read-only; walks every library playlist (slow on big libraries) |
| 4 | `print` | Write those tracks out | src/cli.py:136 → `print_tracks` (src/ytmusic.py:310) | Writes `tracks.txt` (artist/title/videoId, TSV) in CWD |
| 5 | `playlist_map` | Update playlist map | src/cli.py:140 → `update_playlists_map` (src/ytmusic.py:342) | **Writes `1.yaml` (hardcoded)**; excludes `LM`/`SE` |
| 6 | `distribute` | Add out-of-playlist tracks to playlists | src/cli.py:142 → `distribute_tracks` (src/ytmusic.py:326) | **Mutates YouTube playlists** based on `playlists_map.yaml` artist match |
| 7 | `download` | Download all playlists | src/cli.py:144 → `download_all_playlists` (src/ytmusic.py:496) | yt-dlp + SOCKS5; writes `downloads/<Playlist>/` and `track_map_*.yaml`; skips `LM`/`SE`; resumable |
| 8 | `download_track` | Download one track | src/cli.py:146 → `download_track` (src/ytmusic.py:403) | **Hardcoded demo video id `9zhK-QaEYZY`**; saves to `downloads/` |
